...
function NavItemComponent({ item, isActive, onClick, showBadge, badgeCount }: NavItemComponentProps) {
  return (
    <button
      onClick={onClick}
      className={`flex items-center justify-between w-full px-3 py-2 text-sm rounded-lg transition-all duration-150 ${
        isActive
          ? 'bg-indigo-50 text-indigo-700 font-medium border-l-2 border-indigo-600 -ml-0.5 pl-[10px]'
          : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900 border-l-2 border-transparent -ml-0.5 pl-[10px]'
      }`}
    >
      ...
      {isActive && (
        <span className="w-1.5 h-1.5 bg-indigo-600 rounded-full ml-2" />
      )}
    </button>
  )
}
