...
export function useSafeList(options: UseSafeListOptions) {
  const { doctype, pageSize = 20, enabled = true, filters } = options
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // Fetch rows
      const rowsData = await fetchSafeListRows(doctype, page, currentPageSize, filters)
  ...
  }, [doctype, enabled, page, currentPageSize, filters])
