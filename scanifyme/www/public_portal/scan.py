...
def get_context():
	...
	if result.get("error"):
		...
		context.error = "invalid"
		context.message = "This QR code is not valid or has expired."
		context.item = None
		return context
	...
	if not qr_tag or not registered_item:
		...
		context.message = "This item is currently unavailable."
		context.item = None
		return context
	...
	if item_status not in ["Active"]:
		...
		context.error = "unavailable"
		context.message = f"This item is currently marked as {item_status}."
		context.item = None
		return context
