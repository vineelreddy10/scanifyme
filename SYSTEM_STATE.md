...
# Critical Deployment Lessons (2026-03-22)

## Lesson 1: Frappe Asset Volume Mismatch in Docker

### Problem
After rebuilding the Docker image, the desk interface showed 404 errors for CSS files:
```
GET /assets/frappe/dist/css/desk.bundle.XXXX.css HTTP/1.1" 404
GET /assets/frappe/dist/css/report.bundle.XXXX.css HTTP/1.1" 404
```

### Root Cause
The backend and frontend containers were using **different Docker volumes** for the assets directory:

- **Backend container**: Has an anonymous volume mounted at `/home/frappe/frappe-bench/sites/assets`
- **Frontend container**: Uses the shared `scanifyme_sites` volume

When `bench build` runs in the backend container, it writes new CSS files to the anonymous volume, which the frontend cannot access.

### Solution
After running `bench build` in the backend container, copy the new assets to the frontend:
...
docker exec scanifyme-backend-1 bench --site scanifyme.app clear-website-cache
```

### Validation After Deployment
```bash
# Check for 404 errors in frontend logs
docker logs scanifyme-frontend-1 --tail 50 2>&1 | grep -E '404|500'

# Verify CSS files exist in frontend
docker exec scanifyme-frontend-1 ls /home/frappe/frappe-bench/sites/assets/frappe/dist/css/ | head -10

# Test desk interface loads
curl -s -c /tmp/cookies.txt -X POST http://localhost:8080/api/method/login \
  -H 'Content-Type: application/json' \
  -d '{"usr":"Administrator","pwd":"admin@123"}' && \
curl -s -b /tmp/cookies.txt http://localhost:8080/desk | grep -o 'desk.bundle.[A-Z0-9]*.css'
```

### Prevention Rule
**ALWAYS** after running `bench build` in the backend container during deployment:
1. Copy assets from backend to frontend container
2. Clear caches
3. Verify no 404 errors in frontend logs
4. Test desk interface loads correctly

---

## Lesson 2: Safe List API Filter Type Mismatch

### Problem
Frontend was sending `filters` as a dict, but backend expected `str | None`:
```
Argument 'filters' in 'scanifyme.api.safe_list_api.get_safe_list_rows' should be of type 'str | None' but got 'dict' instead.
```

### Solution
Changed backend type annotation to accept both:
```python
# Before
filters: str = None
...
### File Modified
- `scanifyme/api/safe_list_api.py` — Line 217

---

# Deployment Constraints (POC Environment)

## VPS Specifications

| Resource | Specification | Notes |
|----------|---------------|-------|
| **CPU** | 2 vCPU | Limited parallel processing |
| **RAM** | 4 GB | May cause memory pressure during builds |
| **Disk** | 40 GB total | ~30GB free after cleanup |
...
### Disk Space Risks
- **Docker builds can exhaust disk** — Image layers + build cache consume ~19GB
- **Build cache is the biggest offender** — Must be cleaned after each build
- **Dangling images accumulate** — Each rebuild leaves ~2.3GB orphaned layers

### Memory Constraints
- **4GB RAM limits concurrent builds** — Avoid parallel container operations
- **Node.js builds may fail** — Monitor memory during `bench build`

### Build Time
- **Rebuilds take 5-10 minutes** — Due to limited CPU
- **Avoid --no-cache unless necessary** — Saves time and disk

## Disk Monitoring Checklist

### Before Deployment
```bash
ssh root@91.107.206.65 "df -h / && docker system df"
```

**Thresholds:**
- ✅ **>15GB free**: Safe to deploy
- ⚠️ **10-15GB free**: Clean up first
- 🚨 **<10GB free**: MUST clean up before deploy
...
# Frees orphaned image layers from rebuilds
```

### Remove Unused Images
```bash
docker image prune -a -f
# Removes ALL images not used by running containers
# ⚠️ Use with caution — will remove cached base images
```

### Remove Build Cache
```bash
docker builder prune -a -f
# Frees build cache (biggest disk consumer ~19GB)
```
...
| Resource | Command | Risk |
|----------|---------|------|
| **Volumes** | `docker volume prune` | ❌ WILL DELETE DATABASE |
| **Running containers** | `docker rm <id>` | ❌ WILL STOP SERVICES |
| **Active images** | `docker rmi scanifyme:v16` | ❌ WILL BREAK DEPLOYMENT |

### Verify Before Volume Cleanup
```bash
# List volumes
docker volume ls

# Check which are active
docker inspect scanifyme-db-1 | grep -A 10 Mounts

# NEVER prune volumes without explicit verification
```

## Cleanup Results (2026-03-22)

**Before Cleanup:**
- Disk: 9.6GB free (74% used)
- Docker: 2.38GB reclaimable images + 19.43GB build cache

**After Cleanup:**
- Disk: 30GB free (16% used)
- Docker: 0B reclaimable

**Space Reclaimed: ~21GB**

---

# Safe Redeploy Workflow (Low Resource VPS)

## Pre-Deploy Checklist

- [ ] Check disk space: `df -h /` → Must have >15GB free
- [ ] Check Docker usage: `docker system df`
- [ ] Clean up if needed (see above)
- [ ] Verify all changes committed to GitHub main
...
### 3. Rebuild Image (only if code changed)
```bash
# Use --no-cache ONLY if necessary (saves disk but slower)
docker build \
...
# Check for errors
docker logs scanifyme-frontend-1 --tail 50 | grep -E '404|500'
...
|-----------|--------------|
| Backend | `docker logs scanifyme-backend-1` |
| Frontend (Nginx) | `docker logs scanifyme-frontend-1` |
| Database | `docker logs scanifyme-db-1` |
| WebSocket | `docker logs scanifyme-websocket-1` |
...
docker inspect scanifyme-backend-1 | jq -r '.LogPath'

# Truncate (safe for running containers)
...
To limit log growth, add to compose.yaml:
```yaml
services:
  backend:
    logging:
...
# Post-Deploy Operational Validation (2026-03-23)

## Executive Summary

**Status**: ✅ ALL SYSTEMS OPERATIONAL

Comprehensive post-deployment validation completed on the VPS (91.107.206.65). All infrastructure, APIs, frontend routes, QR management flow, and RBAC are functioning correctly.
...
|-----------|--------|--------|-------|
| scanifyme-frontend-1 | ✅ Running | 16+ hours | Nginx serving correctly |
| scanifyme-backend-1 | ✅ Running | 16+ hours | Gunicorn healthy |
| scanifyme-websocket-1 | ✅ Running | 16+ hours | Socket.IO active |
| scanifyme-queue-short-1 | ✅ Running | 16+ hours | Worker active |
| scanifyme-queue-long-1 | ✅ Running | 16+ hours | Worker active |
| scanifyme-scheduler-1 | ✅ Running | 16+ hours | Cron jobs active |
| scanifyme-db-1 | ✅ Healthy | 16+ hours | MariaDB 11.8 |
| scanifyme-redis-cache-1 | ✅ Running | 16+ hours | Cache active |
| scanifyme-redis-queue-1 | ✅ Running | 16+ hours | Queue active |
| buildx_buildkit_scanifyme-builder0 | ✅ Running | 2+ days | BuildKit |

### Resource Usage
| Resource | Used | Available | Status |
|----------|------|-----------|--------|
| **Disk** | 5.8GB (17%) | 30GB free | ✅ Healthy |
| **Memory** | 1.9GB | 1.9GB available | ✅ Acceptable |
| **CPU** | 0% idle | 2 vCPU | ✅ Idle |
| **Docker Images** | 2.98GB | 0B reclaimable | ✅ Clean |
| **Build Cache** | 0B | 0B | ✅ Clean |

## Live Route Validation Matrix

| Route | Status | Response Time | Notes |
|-------|--------|---------------|-------|
| `/app` | ✅ Working | <1s | Frappe Desk loads |
| `/frontend` | ✅ Working | <1s | React SPA loads |
| `/frontend/list/QR%20Batch` | ✅ Working | <1s | List loads |
| `/frontend/list/QR%20Code%20Tag` | ✅ Working | <1s | List loads |
| `/frontend/list/QR%20Code%20Tag?batch=QRB-2026-00001` | ✅ Working | <1s | Filtered list works |
| `/frontend/m/QR%20Code%20Tag/<name>` | ✅ Working | <1s | Detail page loads |
| `/frontend/items` | ✅ Working | <1s | Items page loads |
| `/frontend/recovery` | ✅ Working | <1s | Recovery page loads |
| `/frontend/notifications` | ✅ Working | <1s | Notifications load |

## Live API Validation Matrix

| API | Status | Response | Notes |
|-----|--------|----------|-------|
| `frappe.auth.get_logged_user` | ✅ Working | JSON | Auth working |
| `frappe.desk.form.load.getdoctype` | ✅ Working | JSON | Schema loads |
| `scanifyme.api.safe_list_api.get_safe_list_rows` | ✅ Working | JSON | Lists work |
| `scanifyme.api.safe_list_api.get_safe_list_schema` | ✅ Working | JSON | Schema works |
| `scanifyme.qr_management.api.qr_api.get_qr_batch_detail` | ✅ Working | JSON | Batch detail works |
| `scanifyme.qr_management.api.qr_api.get_qr_tag_detail` | ✅ Working | JSON | Tag detail works |
| `scanifyme.reports.api.dashboard_api.get_owner_dashboard_summary` | ✅ Working | JSON | Dashboard works |
| `scanifyme.notifications.api.notification_api.get_unread_notification_count` | ✅ Working | JSON | Notifications work |
| `scanifyme.onboarding.api.onboarding_api.get_owner_onboarding_state` | ✅ Working | JSON | Onboarding works |

## QR Management Validation Results

### Flow Tested
1. ✅ QR Batch list loads (1 batch: QRB-2026-00001)
2. ✅ QR Code Tag list loads (7 tags)
3. ✅ Filtered QR list with batch filter works
4. ✅ QR detail page loads
5. ✅ QR URL generated correctly
6. ✅ QR batch detail API returns correct data

### Data Verified
- **QR Batches**: 1 batch (QRB-2026-00001)
- **QR Code Tags**: 7 tags with various statuses
- **QR Images**: Not generated for demo data (expected)
- **Filter Contract**: Dict format works: `{"batch": "QRB-2026-00001"}`

### Known Behavior
- QR images are `null` for demo data (images not generated during demo data creation)
- This is expected - images are generated when batch is explicitly generated via API

## RBAC Validation Results

### Admin (Administrator)
- ✅ Can access all QR management APIs
- ✅ Can access owner dashboard APIs
- ✅ Can access all list/detail pages
- ✅ Notification count: 6 unread

### Owner
- ✅ Can access owner dashboard
- ✅ Can view own items
- ✅ Can view own recovery cases
- ✅ Onboarding state: 100% complete

### Guest
- ❌ Cannot access authenticated APIs (expected)
- ✅ Only sees public scan pages

## Frontend Stability Validation

### Verified Working
- ✅ No React runtime crashes
- ✅ No `searchQuery` errors
- ✅ No filter contract mismatches
- ✅ No `.toFixed()` crashes
- ✅ No `Invalid URL` errors
- ✅ No `/frontend/api/*` issues
- ✅ Sidebar loads correctly
- ✅ Route transitions work
- ✅ Dynamic list pages load
- ✅ Dynamic detail pages load

### Asset Loading
- ✅ CSS files load correctly (desk.bundle.IZITBZ7E.css)
- ✅ JS files load correctly
- ✅ No 404 errors in frontend logs
- ✅ No 500 errors in frontend logs

## Low-Resource VPS Stability
...
### Container Restarts
- **Count**: 0 unexpected restarts
- **Status**: ✅ Stable

### Log Growth
- **Backend logs**: Normal
- **Frontend logs**: Normal (no 404/500 errors)
- **Status**: ✅ Stable

## Operational Warnings

### ⚠️ Memory Pressure
- 4GB RAM is tight for concurrent operations
- Monitor during builds and migrations
- Consider adding swap if needed

### ⚠️ Disk Space
- Build cache was consuming ~19GB before cleanup
- Must clean build cache after each rebuild
- Monitor disk before each deployment

### ⚠️ QR Images
- Demo data does not include QR images
- Images must be generated explicitly via batch generation API
- This is expected behavior, not a bug

## Post-Deploy Smoke Test Procedure
...
# Check for errors
docker logs scanifyme-frontend-1 --tail 20 | grep -E '404|500'
...
1. Open `http://91.107.206.65:8080/frontend` in browser
2. Verify sidebar loads
3. Navigate to QR Batch list
4. Click a batch → QR tag list should load
5. Click a tag → QR detail page should load
6. Navigate to Items page → should load
7. Navigate to Recovery page → should load
8. Check browser console for errors

### API Validation
```bash
# Login
curl -c /tmp/cookies.txt -X POST http://localhost:8080/api/method/login \
  -H 'Content-Type: application/json' \
  -d '{"usr":"Administrator","pwd":"admin@123"}'
...
ScanifyMe has automated deployment via GitHub Actions. Pushing to `main` branch triggers automatic deployment to the VPS.
...
GitHub Actions Workflow
    ↓
SSH to VPS (91.107.206.65)
    ↓
Preflight Checks (disk, docker, files)
    ↓
Docker Cleanup (low-resource safety)
    ↓
Build Image (pulls latest scanifyme from GitHub)
    ↓
Restart Services
    ↓
Run Migrations
...
Clear Caches
    ↓
Smoke Tests (API, frontend, QR flow)
    ↓
Post-deploy Cleanup
    ↓
...
| File | Purpose |
|------|---------|
| `deploy/deploy.sh` | Main deployment script with preflight, build, migrate, smoke tests |
| `deploy/rollback.sh` | Rollback helper for diagnostics and recovery |
...
2. Select "Deploy ScanifyMe to VPS" workflow
3. Click "Run workflow"
4. Optionally skip smoke tests (not recommended)

#### Option 3: Manual SSH Deploy
```bash
ssh root@91.107.206.65
cd /opt/scanifyme
# Copy deploy.sh to VPS or run directly
bash deploy.sh
```
...
| `VPS_SSH_KEY` | SSH private key (PEM format) | `-----BEGIN OPENSSH PRIVATE KEY-----...` |

### Preflight Validation Rules

Before every deployment, the system checks:

1. ✅ Deployment directory exists (`/opt/scanifyme`)
2. ✅ Docker is installed and running
3. ✅ Docker Compose is available
4. ✅ Required files exist (compose.prod.yaml, .env, apps.json)
5. ✅ Minimum 10GB disk space available
6. ✅ Docker daemon is healthy

If any check fails, deployment aborts with clear error message.

### Post-Deploy Smoke Tests

After deployment, these tests run automatically:

| Test | What It Validates |
|------|-------------------|
| API Ping | Backend is responding |
| Frontend Load | React SPA loads |
| Login | Authentication works |
| QR Batch List | List API returns data |
| Filtered QR List | Filter contract (dict format) works |
| Owner Dashboard | Owner APIs work |
| Container Health | No unhealthy containers |
| Restart Loops | No containers restarting |

### Low-Resource Cleanup Strategy

The deployment includes automatic cleanup for the constrained VPS:

| Action | When | Purpose |
|--------|------|---------|
| `docker image prune -f` | Pre-deploy | Remove dangling images |
| `docker container prune -f` | Pre-deploy | Remove stopped containers |
| `docker network prune -f` | Pre-deploy | Remove unused networks |
| `docker builder prune -a -f` | Pre-deploy & Post-deploy | Remove build cache (~19GB) |

**⚠️ CRITICAL**: Volumes are NEVER pruned (would delete database).

### Rollback / Failure Handling

#### If Deployment Fails

1. **Preflight failure**: Check error message, fix issue, retry
2. **Build failure**: Current system remains running, no action needed
3. **Migration failure**: Check logs, may need manual intervention
4. **Smoke test failure**: Deployment marked as failed, check logs
...
ssh root@91.107.206.65 "docker ps"

# Check disk space
ssh root@91.107.206.65 "df -h /"

# View backend logs
ssh root@91.107.206.65 "docker logs scanifyme-backend-1 --tail 50"

# View frontend logs
ssh root@91.107.206.65 "docker logs scanifyme-frontend-1 --tail 50"

# Test API
curl http://91.107.206.65:8080/api/method/ping
```
...
1. **Option 1**: Rename workflow file
   ```bash
   mv .github/workflows/deploy.yml .github/workflows/deploy.yml.disabled
   ```

2. **Option 2**: Delete workflow file
   ```bash
   rm .github/workflows/deploy.yml
   ```

3. **Option 3**: Remove GitHub secrets (workflow will fail safely)
...
Use this checklist for **every** VPS deployment to avoid asset and configuration issues.

## Pre-Deployment
- [ ] All changes committed to GitHub main branch
- [ ] Local tests passing
- [ ] SYSTEM_STATE.md updated with new features/changes

## Deployment Steps
- [ ] SSH into VPS: `ssh root@91.107.206.65`
- [ ] Navigate to deployment directory: `cd /opt/scanifyme`
- [ ] Clean Docker cache if disk space low: `docker system prune -a -f`
- [ ] Rebuild image with latest code:
  ...
  docker build \
    ...
    --load
  ```
- [ ] Restart containers: `docker compose --env-file .env -f compose.prod.yaml up -d`
- [ ] Run migrations: `docker exec scanifyme-backend-1 bench --site scanifyme.app migrate`
- [ ] Build assets: `docker exec scanifyme-backend-1 bench --site scanifyme.app build`

## ⚠️ CRITICAL: Asset Synchronization
After `bench build`, **MUST** copy assets from backend to frontend:
```bash
# Copy CSS files
docker cp scanifyme-backend-1:/home/frappe/frappe-bench/sites/assets/frappe/dist/css/. /tmp/css_backend/
docker cp /tmp/css_backend/. scanifyme-frontend-1:/home/frappe/frappe-bench/sites/assets/frappe/dist/css/
...
# Copy scanifyme app assets
docker cp scanifyme-backend-1:/home/frappe/frappe-bench/sites/assets/scanifyme/. /tmp/scanifyme_assets/
docker cp /tmp/scanifyme_assets/. scanifyme-frontend-1:/home/frappe/frappe-bench/sites/assets/scanifyme/
```
...
- [ ] Clear caches:
  ...
  docker exec scanifyme-backend-1 bench --site scanifyme.app clear-website-cache
  ```
- [ ] Check for 404 errors: `docker logs scanifyme-frontend-1 --tail 50 | grep -E '404|500'`
- [ ] Test API ping: `curl http://localhost:8080/api/method/ping`
- [ ] Test login: `curl -X POST http://localhost:8080/api/method/login -H 'Content-Type: application/json' -d '{"usr":"Administrator","pwd":"admin@123"}'`
- [ ] Test desk interface: Open `http://91.107.206.65:8080/desk` in browser
- [ ] Test frontend: Open `http://91.107.206.65:8080/frontend` in browser
- [ ] Test filtered list: Open `http://91.107.206.65:8080/frontend/list/QR%20Code%20Tag?batch=QRB-2026-00001`
- [ ] Verify all containers running: `docker ps`
- [ ] Check container logs for errors: `docker logs scanifyme-backend-1 --tail 100`

## Common Issues & Fixes

### Issue: CSS/JS files return 404
**Cause**: Asset volume mismatch between backend and frontend containers
**Fix**: Copy assets from backend to frontend (see Asset Synchronization above)

### Issue: Filters type mismatch error
**Cause**: Backend type annotation too restrictive
**Fix**: Ensure `filters: str | dict | None = None` in API signatures

### Issue: Disk space error during build
**Cause**: Docker cache consuming too much space
**Fix**: `docker system prune -a -f` before rebuilding

---

# ScanifyMe Deployment Architecture (Complete Pipeline Audit)

## 1. Executive Summary

This document provides complete visibility into the ScanifyMe deployment pipeline, from GitHub push to VPS execution. The deployment uses frappe_docker to build a custom Docker image that includes the ScanifyMe app, then orchestrates the container stack via Docker Compose.

### High-Level Architecture

```
GitHub Push (main) → GitHub Actions → SSH to VPS → 10-Step Deployment
                                                        │
                                                        ▼
                                              ┌─────────────────┐
                                              │ 1. Preflight    │
...
## 2. CI/CD Flow - GitHub Actions Pipeline

### Trigger Conditions

| Trigger | Branch | Manual Option |
|---------|--------|---------------|
| Push to | `main` | Yes (workflow_dispatch) |

### Workflow File
**Location**: `.github/workflows/deploy.yml`

### Jobs and Steps (30 min timeout)
1. Checkout code
2. Setup SSH key (from secrets)
3. Verify VPS connectivity
4. Check VPS resources
5. Run preflight checks (disk ≥10GB, Docker running, files exist)
6. Clean Docker resources
7. Build and deploy (docker build + docker compose up)
8. Run migrations
9. Build and sync assets
10. Run smoke tests (8 tests)
11. Post-deploy cleanup
12. Success/Failure report

### Secrets Used

| Secret | Purpose |
|--------|---------|
| `VPS_HOST` | VPS IP (91.107.206.65) |
| `VPS_USER` | SSH user (root) |
| `VPS_SSH_KEY` | SSH private key |

### SSH Mechanism
- GitHub Actions writes private key to `~/.ssh/deploy_key`
- Uses `ssh -i ~/.ssh/deploy_key user@host "commands"`

### Exact Flow
```
git push → GitHub webhook → Checkout → SSH setup → 
SSH commands execute deploy.sh on VPS → Results returned
```

### Error Handling
- **Preflight failure**: Abort with error message
- **Build failure**: Current system remains running
- **Migration failure**: Manual intervention required
- **Smoke test failure**: Marked as failed, rollback guidance provided

---

## 3. VPS Deploy Script Breakdown

### File: `deploy/deploy.sh` (451 lines)

### Execution Order (10 Steps):

1. **Preflight Checks** - Verify directory, Docker, files, disk ≥10GB, daemon running
2. **Cleanup Docker** - Remove images, containers, networks, build cache (NEVER volumes)
3. **Build Image** - docker build with FRAPPE_PATH, FRAPPE_BRANCH, APPS_JSON_BASE64
4. **Restart Services** - docker compose up -d
5. **Run Migrations** - bench migrate
6. **Build Assets** - bench build
7. **Sync Assets** - Copy CSS/JS from backend to frontend (CRITICAL)
8. **Clear Caches** - clear-cache + clear-website-cache
9. **Smoke Tests** - 8 tests
10. **Post-deploy Cleanup** - Clean build cache

### Failure Points:

| Step | Risk | Handling |
|------|------|----------|
| Preflight | Abort if <10GB disk | Must clean first |
| Build | May fail, system continues | Retry |
| Migrate | May break site | Manual intervention |
| Sync | 404 errors if skipped | Manual sync fallback |

---

## 4. Docker Architecture

### Docker Files (On VPS - part of frappe_docker)

| File | Purpose |
|------|---------|
| `/opt/scanifyme/images/custom/Containerfile` | Custom image build |
| `/opt/scanifyme/compose.yaml` | Base compose |
| `/opt/scanifyme/compose.prod.yaml` | Production compose |
| `/opt/scanifyme/.env` | Environment variables |
| `/opt/scanifyme/apps.json` | App configuration |

### Container List (10 services)

| Container | Role | Ports |
|-----------|------|-------|
| scanifyme-db-1 | MariaDB | 3306 |
| scanifyme-redis-cache-1 | Redis cache | 6379 |
| scanifyme-redis-queue-1 | Redis queue | 6379 |
| scanifyme-redis-socketio-1 | Redis Socket.IO | 6379 |
| scanifyme-backend-1 | Gunicorn/WSGI | - |
| scanifyme-frontend-1 | Nginx proxy | 8080:80 |
| scanifyme-websocket-1 | Socket.IO | 9000 |
| scanifyme-queue-short-1 | Short job worker | - |
| scanifyme-queue-long-1 | Long job worker | - |
| scanifyme-scheduler-1 | Cron scheduler | - |

### Request Flow

```
Browser → :8080 → Nginx (frontend) → Backend (Gunicorn) 
                                            ↓
                                      MariaDB / Redis
```
...
| Volume | Shared By |
|--------|-----------|
| scanifyme_sites | backend, frontend, websocket, workers, scheduler |
| scanifyme_db-data | db only (NEVER prune) |
...
### Which Image
- **Base**: frappe/frappe:version-16 (from Docker Hub)
- **Custom**: scanifyme:v16 (built on VPS)

### Build Process (5 Stages)

1. **Base Image** - Pull frappe/frappe:version-16
2. **Bench Creation** - Clone frappe, install bench
3. **App Installation** - bench get-app from apps.json (scanifyme from GitHub)
4. **Dependencies** - pip install, npm install
5. **Finalize** - Tag image, set entrypoint
...
### How Scanifyme Gets Installed

- **At build time**: Containerfile runs `bench get-app` + `bench install-app`
- **Inside container**: `/home/frappe/frappe-bench/sites/apps/scanifyme/`

### Installation Strategy

- Uses `bench get-app` during image build
- App is baked into image (not installed at runtime)
- To update: rebuild image

---

## 7. Migration & Assets Flow

### Migration
- **Runs**: After every deployment (Step 5 of deploy.sh)
- **Command**: `bench --site scanifyme.app migrate`
- **Frequency**: Every deploy

### Asset Build
- **Command**: `bench --site scanifyme.app build`
- **Output**: `/home/frappe/frappe-bench/sites/assets/frappe/dist/`

### Asset Sync (CRITICAL)
- **Problem**: Backend uses anonymous volume, frontend uses shared volume
- **Solution**: Copy CSS/JS from backend to frontend after build
- **Without this**: 404 errors for all CSS/JS files

### Frontend Serving
- Served by Nginx in frontend container
- Routes like /frontend/* handled by Frappe bench
- Static assets from shared volume

---

## 8. Smoke Test System

### Location: `deploy/smoke_tests()` function (lines 261-365)

### 8 Tests Executed:

| # | Test | Pass Condition |
|---|------|----------------|
| 1 | API Ping | Contains "pong" |
| 2 | Frontend Load | Contains "doctype html" |
| 3 | Login | Contains "Logged In" |
| 4 | QR Batch List | Contains "rows" |
| 5 | Filtered QR List | Contains "rows" |
| 6 | Owner Dashboard | Contains "items" |
| 7 | Container Health | 0 unhealthy |
| 8 | Restart Loops | 0 restarting |

### Pass/Fail Logic

- If any test fails: `failed=1` → deployment marked failed
- Can skip via workflow_dispatch (not recommended)
...
### What It Does (NOT true rollback):

| Command | Action |
|---------|--------|
| status | Show containers, disk, logs |
| restart | Stop/start all containers |
| logs | Show recent logs |
| diagnose | Run diagnostic checks |

### Previous Image Preservation
- **NOT preserved** - Previous image deleted after rebuild
- **If build fails** - Old system continues running

### Manual Rollback
- No automatic rollback - manual intervention required
- If migration fails - may need DB restore

---

## 10. Reproducibility

### Can New VPS Replicate? YES

### Required Inputs:

1. VPS with Docker (2 vCPU, 4GB RAM, 40GB disk)
2. SSH access configured
3. GitHub repo with deploy.yml
4. GitHub secrets: VPS_HOST, VPS_USER, VPS_SSH_KEY

### Fresh Setup Steps:

1. Provision VPS with Docker
2. Clone frappe_docker to /opt/scanifyme
3. Create .env, apps.json
4. Build image
5. Start containers
6. Create site and install app
7. Configure GitHub secrets
8. Push to main to trigger deploy

### Hidden Dependencies:

- frappe_docker main branch on VPS
- apps.json must match repo
...
| Risk | Severity | Mitigation |
|------|----------|------------|
| Disk exhaustion | 🔴 High | Pre-deploy cleanup, 10GB minimum |
| Docker cache growth | 🔴 High | Clean after every deploy |
| Migration failure | 🔴 High | Test locally first |
| Partial deploy state | 🟡 Medium | Smoke tests catch issues |
| Secrets leakage | 🔴 High | Rotate keys regularly |
| Container restart loops | 🟡 Medium | Monitor with smoke tests |
| Build failures | 🟡 Medium | Retry mechanism |
| Asset sync failure | 🟡 Medium | Manual sync fallback |
...
## 12. Master Documentation Summary
...
| deploy/deploy.sh | Repo root + VPS |
| deploy/rollback.sh | Repo root + VPS |
| SYSTEM_STATE.md | Repo root |
...
# Deploy (manual)
ssh root@91.107.206.65 "cd /opt/scanifyme && ./deploy/deploy.sh"

# Check status
ssh root@91.107.206.65 "cd /opt/scanifyme && ./deploy/rollback.sh status"

# View logs
ssh root@91.107.206.65 "docker logs scanifyme-backend-1 --tail 100"
```

---

## 13. How Deployment Works Internally (Complete Flow)

```
DEVELOPER: git push origin main
    │
    ▼
GITHUB: trigger deploy.yml workflow
    │
    ▼
CHECKOUT: Clone repo
    │
    ▼
SSH SETUP: Write private key, add to known_hosts
    │
    ▼
VPS PREFLIGHT: Check disk ≥10GB, Docker running, files exist
    │
    ▼
VPS CLEANUP: Remove images, containers, networks, build cache
    │
    ▼
VPS BUILD: docker build with frappe + scanifyme (4 min)
    │
    ▼
VPS START: docker compose up -d (10 containers)
    │
    ▼
VPS MIGRATE: bench migrate
    │
    ▼
VPS BUILD ASSETS: bench build (1-2 min)
    │
    ▼
VPS SYNC ASSETS: Copy CSS/JS backend → frontend (CRITICAL)
    │
    ▼
VPS CLEAR CACHE: clear-cache + clear-website-cache
    │
    ▼
VPS SMOKE TESTS: 8 tests (API, frontend, login, QR, health)
    │
    ▼
VPS CLEANUP: Remove build cache
    │
    ▼
COMPLETE: Site available at http://91.107.206.65:8080
```
